...
@if(Auth::Check() and $auth=Auth::User())
    ...
    <div class="alert alert-success col-md-offset-2 col-lg-7 " xmlns="http://www.w3.org/1999/html">
        ...
        <form action="{{URL::to("update")}}" method="post" >
            ...
            <b class="col-md-offset-4 alert alert-warning "><i class="glyphicon glyphicon-pencil"> Update Point </i></b>

            <hr/>

            <input type="hidden" name="id" value="{{$user->id}}" placeholder=" Student Registration Number" class="form-control " required/>
            <input type="text" name="regno" value="{{$user->regno}}" placeholder=" Student Registration Number" class="form-control " required/>
            @if($errors->has('regno'))
                <div class="alert alert-danger"> {{$errors->first('regno')}}</div>
            @endif
            <br/>
            <input type="text" name="u_name"  value="{{$user->u_name}}" placeholder=" Student or User Name" class="form-control" required/>
            ...
            @endif
            <hr/>
            <input type="text" name="email"  value="{{$user->email}}" placeholder=" Student Email" class="form-control" required/>
            @if($errors->has('email'))
                <DIV class="alert alert-danger"> {{$errors->first('email')}}</DIV>
            @endif
            <br/>
            <input type="password" name="password"  value="{{$user->password}}" placeholder=" Password.  (Recommended Use Your Regno i.e BIT/28/301/DT) " class="form-control" required/>
            <hr/>
            ...
            <button type="submit" class="btn btn-success col-md-offset-4"><b class="glyphicon glyphicon-send">Save Changes</b></button>
            <button   class="btn btn-info col-md-offset-4" ><b class="glyphicon glyphicon-refresh"> <a href="{{URL::to('UserList')}}"></a> Cancel</b></button>
        </form>
